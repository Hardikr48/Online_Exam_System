		...
		<c:forEach items="${sessionScope.collegedata }" var="q">
			...
			<li><a
				href="<%=request.getContextPath()%>/Subject?flag=insert&id=${q.id }">Add
					Subject</a></li>
			<li><a
				href="<%=request.getContextPath()%>/Professor?flag2=professor&flag=insert&id=${q.id }">Add
					Professor</a></li>
			<li><a
				href="<%=request.getContextPath()%>/Professor?flag2=hod&flag=insert&id=${q.id }">Add
					Head Of Department</a></li>
			<li><a
				href="<%=request.getContextPath()%>/Student?flag=insert&id=${q.id }">Add
					Student</a></li>
			<li><a
				href="<%=request.getContextPath()%>/Exam?flag=insert&id=${q.id }">Add
					Exam</a></li>
			<li><a
			...
			<li><a
				href="<%=request.getContextPath()%>/Subject?flag=searchcollegesubject&id=${q.id }">View
					Subject</a></li>
			<li><a
				href="<%=request.getContextPath()%>/Professor?flag=searchcollegeprofessor&id=${q.id }">View
					Professor</a></li>
			<li><a
				href="<%=request.getContextPath()%>/Student?flag=searchcollegestudent&id=${q.id }">View
					Student</a></li>
			<li><a
				href="<%=request.getContextPath()%>/Exam?flag=searchcollegeexam&id=${q.id }">View
					Exam</a></li>
		...
		<div style="padding-top: 3%;">
			...
			<form action="<%=request.getContextPath()%>/Exam" method="post">
				...
				<c:forEach items="${sessionScope.examlist }" var="q">
					<%=i++%>
			Phase<%=i%>:-<input type="text" name="phase" value="${q.phase }"
						required>
					<br>
					<br>
			Time<%=i%>:-<input type="text" name="time" value="${q.time }"
						required>
					<br>
					<br>
			Marks<%=i%>:-<input type="email" name="maeks" value="${q.marks }"
						required>
					<br>
					<br>
					<input type="hidden" name="id" value="${q.id}" required>
					<input type="hidden" name="departmentid"
						value="${q.departmentid.id }" required>
					<input type="hidden" name="semid" value="${q.semesterid.id }"
						required>
					<input type="hidden" name="semid" value="${q.subjectid.id }"
						required>
