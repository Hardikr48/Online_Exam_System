...
<div style="margin-left:25%;padding:1px 16px;height:1000px;">
	<div style="padding-top:2%;" >
		...
		<%int i=1; %>
		<a class="previous"
			href="College_Professor_List.jsp">&laquo;
			Previous</a>
		<table border="1">
			<tr>
				<td>No</td>
				...
				<c:forEach items="${sessionScope.professordepartmentlist }" var="q">
					<tr>
						<td><%=i %></td>
						<td>${q.professorid.firstName }</td>
						<td>${q.departmentid.department }</td>
						<td><a href="<%=request.getContextPath()%>/Professor?flag=viewCollegeProfessorDepartmentSemList&id=${q.id }">View Semester</a></td>
						<td><a href="<%=request.getContextPath()%>/Professor?flag=viewCollegeProfessorDepartmentSubjectList&id=${q.id }">View Subject</a></td>
					</tr>
					<%i++; %>
				</c:forEach>
		</table>
